...
export async function renderMeta(container) {
  ...
  container.innerHTML = `
    <div class="meta-layout">
      ...
      <!-- Page header -->
      <div class="meta-page-header glass-panel" style="display:flex;justify-content:space-between;align-items:flex-start">
        <div style="display:flex;gap:30px">
          <div class="meta-page-title">
            <div class="meta-page-icon">📈</div>
            <div>
              <h1 class="meta-h1">Meta Tracker</h1>
              <p class="meta-subtitle">${format} · ${data.source || 'Static'} · Updated ${new Date(updatedAt).toLocaleDateString()}</p>
            </div>
          </div>
          <div class="meta-legend" style="display:flex;flex-wrap:wrap;gap:15px;align-items:center;margin-top:10px">
        ...
        <button id="btn-sync-meta" class="btn btn-primary" style="margin-top:10px">
          🔄 Sync Live Meta
        </button>
      </div>
  ...
  // Sync Meta
  const syncBtn = document.getElementById('btn-sync-meta');
  if (syncBtn) {
    syncBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showToast('Successfully synced live meta from MTGGoldfish', 'success');
        // Re-render
        renderMeta(container);
      } catch (err) {
        syncBtn.disabled = false;
        syncBtn.innerHTML = '🔄 Sync Live Meta';
